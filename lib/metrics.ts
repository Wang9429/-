...
export const INDICATORS: IndicatorDef[] = [
    ...
    leaves: (ctx) =>
      assets().map((a) => ({
        ...
        extras: [
          ...
          { label: "演示预警阈值", value: `${a.utilization_threshold_pct}%` },
        ],
        riskIds: risksFor(a.id, ctx),
        dataComplete: true,
      })),
  ...
  },
  {
    ...
    kind: "count",
    leafObjectType: "fixed_asset_project",
    formula: "当前组织范围内固定资产投资项目按 project_id 去重计数",
  ...
  },
  {
    ...
    kind: "count",
    leafObjectType: "fixed_asset_project",
    formula: "预计完工投资 > 当前有效概算的项目数（按 project_id 去重）",
  ...
  },
  {
    ...
    kind: "count",
    leafObjectType: "fixed_asset_project",
    formula: "截至日仍待核查/核查中/整改中/待复核的唯一 risk_id 数",
    caliber: "按 risk_id 去重；已排除、已关闭不计入。跨领域同一事项不重复计数。",
  ...
  },
  {
    ...
    kind: "count",
    leafObjectType: "equity_project",
    formula: "截至日仍未关闭的唯一 risk_id 数",
    caliber: "按 risk_id 去重；跨领域引用同一事项不重复计数。",
    sourceNote: "本平台监管事项",
  ...
  /* ---------------- 工程项目 ---------------- */
  {
    ...
    formula: "当前组织范围内工程履约项目按 project_id 去重计数",
  ...
  },
  {
    ...
    leafObjectType: "engineering_project",
    formula: "截至日仍未关闭的唯一 risk_id 数",
    caliber: "按 risk_id 去重；同一事项在资金、国际化出现时不重复计数。",
    sourceNote: "本平台监管事项",
    evaluate: (v) => (v === null ? "unknown" : (v as number) > 0 ? "risk" : "normal"),
  ...
  /* ---------------- 资金 ---------------- */
  {
    ...
    formula: "同截至日纳入范围账户余额折人民币合计（原币 × 演示汇率 ÷ 10000）",
    caliber: "余额按截至日统计，不跨期间相加；原币金额、单位及模拟汇率可查。",
    sourceNote: "银行账户余额记录拟来源（模拟），美元汇率 7.2 为演示假设",
    leaves: (ctx) =>
      accounts().map((a) => ({
        objectId: a.id,
  ...
  },
  {
    ...
    formula: "实付金额 > 该笔有效批准金额的付款记录数（按 payment_id 去重）",
    caliber:
  ...
  },
  {
    ...
    leafObjectType: "cash_transaction",
    formula: "截至日仍未关闭的唯一 risk_id 数",
    caliber: "按 risk_id 去重；R03/R04 与股权领域为同一事项。",
    sourceNote: "本平台监管事项",
    evaluate: (v) => (v === null ? "unknown" : (v as number) > 0 ? "risk" : "normal"),
  ...
  /* ---------------- 国际化 ---------------- */
  {
    ...
    kind: "count",
    leafObjectType: "engineering_project",
    formula: "境外工程/投资项目按 project_id 去重计数",
  ...
  },
  {
    ...
    kind: "count",
    leafObjectType: "engineering_project",
    formula: "经人工确认受外部事件影响的项目按 project_id 去重计数",
  ...
  },
  {
    ...
    leafObjectType: "engineering_project",
    formula: "截至日仍未关闭的唯一 risk_id 数",
    caliber: "按 risk_id 去重；R05/R06/R07 与工程、资金领域为同一事项。",
    sourceNote: "本平台监管事项",
    evaluate: (v) => (v === null ? "unknown" : (v as number) > 0 ? "risk" : "normal"),
  ...
  /* ---------------- 产权 ---------------- */
  {
    ...
    formula: "存在有效权益关系记录的被投法人按 legal_entity_id 去重计数",
    caliber:
  ...
  },
  {
    ...
    formula: "当前阶段尚未完成归档跟踪的产权事项按 matter_id 去重计数",
  ...
  },
  {
    ...
    leafObjectType: "property_matter",
    formula: "截至日仍未关闭的唯一 risk_id 数",
    caliber: "按 risk_id 去重；R08 与股权领域为同一事项。",
    sourceNote: "本平台监管事项",
    evaluate: (v) => (v === null ? "unknown" : (v as number) > 0 ? "attention" : "normal"),
