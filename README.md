...
- 固定资产等流程领域用横向肩形箭头联动下区五数；资金管理为上区主体经营财务（盈利/资产负债/流动性，适用卡带小趋势、首页无完整单位表）+ 下区一张「场景执行情况」（默认全部专题，完整 11 项一级目录）；产权管理为上区法人股权全景（四项统计可带快照趋势）+ 下区一张「场景执行情况」（默认全部专题，完整 10 项一级目录；仅产权交易显示经济行为下拉与肩形流程）。国际化用事件/地区。
